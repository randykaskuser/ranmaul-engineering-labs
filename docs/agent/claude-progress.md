...
- Follow-up: the 4 generated covers are now hosted on R2 (`elabs-images` bucket, same as other article covers), rendered with Cloudflare Browser Rendering and uploaded via the Cloudflare API: `https://pub-8985a094178a4352b0af0536c390a51b.r2.dev/<article-slug>.jpg`. MDX `coverImage` points there; `public/assets/covers/` removed. Verified in Cloudflare: 1600x900 JPEG, image/jpeg, sizes within 0.5% of the local renders; title layout inside frame. Still owner action: paste each R2 URL into the Notion page's CoverImage (no Notion token in this environment), otherwise a re-sync drops the cover.
