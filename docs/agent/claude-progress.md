...
- ID Channa article (`content/id/fishkeeping/channa-andrao.mdx`): title "Channa" -> "Channa Andrao: Predator Mini dengan Warna Biru Menyala" (the article's own heading), description "seo" -> real 144-char description, updatedAt 2026-09-25. Also removed a stray ```` ```plain text ```` fence on line 14 that turned the first half of the article (heading, summary, habitat, size table) into one raw-markdown code block. Verified: 10 h2, 1 table, only the 3 intended small code blocks. This page is synced from Notion: the same title/description must be set in Notion and the stray code block removed there, or the next sync PR will revert it.
